...
int main(int argc, char** argv)
{
	...
	while ((c = getopt (argc, argv, ":qv:x:T:N:")) != -1)
	switch (c)
	{
		case 'q':
			quiet = true;
		...
		case 'T':
			...
			break;
		case 'N':
			numSweeps = atoi(optarg);		
			break;
		//case 'f':
		//	maxMutations = atoi(optarg);		
		//	break;
	...
	f.str("");
	f << "./2D_DATA_HOMEOSTASIS/maxSize=" << _maxsize << "_numSweeps=" << numSweeps << "_timeRatio=" << time_fraction << "/seed=" << _seed;
	DIR *dir = opendir(f.str().c_str());
	if(!dir)
	{
		f.str("");
		f << "mkdir -p ./2D_DATA_HOMEOSTASIS/maxSize=" << _maxsize << "_numSweeps=" << numSweeps << "_timeRatio=" << time_fraction << "/seed=" << _seed;
		system(f.str().c_str());
	}
...
*/
	...
	for (int sweep = 0; sweep < numSweeps; ++sweep)
	{
		...
		//cout << "Killing 99\% of all cells" << endl;
		//============================ Kill 50% of cells
		do
		{
		...
		do
		{
			...
			if (iter%10000 == 0)
			{
				...
				if (!quiet) 
				{
					cout << "Sweep #" << sweep << " | Ntot = " << Ntot << " | mutations = " << mutations.size() << " | Birth, Death & Mutation rates -> " << r_birth_normalised << ", " << r_death_normalised << ", " << r_mutation_normalised << " | t = " << t << ", t_max = " << t_max << endl;
				}
	...
	//} while((t < t_max) || (Ntot < _maxsize));
	//} while (mutations.size() < maxMutations);
	//} while (Ntot < _maxsize);
	...
	ofstream liver_file;
	f.str("");
	f << "./2D_DATA_HOMEOSTASIS/maxSize=" << _maxsize << "_numSweeps=" << numSweeps << "_timeRatio=" << time_fraction << "/seed=" << _seed << "/liver.csv";
...
/*
	ofstream VAF_file;
	g.str("");
	g << "./2D_DATA_HOMEOSTASIS/maxSize=" << _maxsize << "_numSweeps=" << numSweeps << "_stemCell=" << stemCell << "_sweeps=" << sweeps
...
/*
	...
	g.str("");
	g << "./DATA/maxSize=" << _maxsize << "_numSweeps=" << numSweeps << "_stemCell=" << stemCell << "_sweeps=" << sweeps << "_ut=" << _ut 
...
/*
	...
	f.str("");
	f << "./DATA/maxSize=" << _maxsize << "_numSweeps=" << numSweeps << "_stemCell=" << stemCell << "_sweeps=" << sweeps << "_ut=" << _ut 
