...
void volumetric_division(Cell ** liver , vector<vector<int> > &mutations , int cell_x , int cell_y , int *Ntot , int *x_b , int *y_b , int radius , int *next_cellID , float *flux , int *flux_count)
{
	...
	/*
	// Compute unit vector pointing from centre cell to dividing cell
	r_x = cell_x - radius;
	r_y = cell_y - radius;

	// Normalise
	r_x /= pow( pow( r_x , 2 ) + pow( r_y , 2 ) + pow( r_z , 2 ) , 0.5 );
	r_y /= pow( pow( r_x , 2 ) + pow( r_y , 2 ) + pow( r_z , 2 ) , 0.5 );
	
	// Compute unit vector pointing from dividing cell to first link in chain 
	v_x = x;
	v_y = y;
	v_z = z;

	// Normalise
	v_x /= pow( pow( v_x , 2 ) + pow( v_y , 2 ) + pow( v_z , 2 ) , 0.5 );
	v_y /= pow( pow( v_x , 2 ) + pow( v_y , 2 ) + pow( v_z , 2 ) , 0.5 );
	v_z /= pow( pow( v_x , 2 ) + pow( v_y , 2 ) + pow( v_z , 2 ) , 0.5 );

	// Compute projection of two vectors and write to file
	scalar_prod = (r_x * v_x) + (r_y * v_y) + (r_z * v_z);
	//outfile << r0 << " " << scalar_prod << endl;

	// Add value to flux array depending on value of r0
	for (int j = 0; j < 40; ++j)
	{
		if (r0 <= ((float)(j+1))*init_rad/40.0)
		{

			//cout << r0 << " is less than " << ((float)(j+1))*init_rad/40.0;
			flux[j] += scalar_prod;
			flux_count[j] += 1;

			//cout << ": " << flux[j] << " " << flux_count[j] << endl;
			break;
		}
	}
	*/
	...
	if (queue > 0)
	{
		...
		//{
			...
			for (int i = 0; i < queue; ++i)
			{
				...
				/*
				// For each link in chain, compute (normalised) vector from (i-1)th link to ith link, and project onto unit vector in radial direction from centre to dividing cell
				// Compute unit vector pointing from dividing cell to first link in chain 
				v_x = chainX[queue-i] - chainX[queue-i-1];
				v_y = chainY[queue-i] - chainY[queue-i-1];
...
int main(int argc, char** argv)
{
	...
	while ((c = getopt (argc, argv, ":qv:x:s:R:m:A:")) != -1)
	switch (c)
	{
		case 'q':
			quiet = true;
		...
		case 'x':
			...
			break;
		case 's':
			stemCell = atoi(optarg);		
			break;
		case 'R':
			stemCell_initialRadius = atof(optarg);		
			break;
		case 'm':
			mutation_ratio = atof(optarg);		
			break;

		case 'A':
			division_ratio = atof(optarg);		
			break;
	...
	//cout << quiet << " " << _seed << " " << stemCell  << " " << mutation_ratio << " " << division_ratio << endl;
	//exit(0);
	...
	f.str("");
	f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps 
			<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "_stemCellRadius=" << stemCell_initialRadius << "/" << _seed << "/animation";
	DIR *dir = opendir(f.str().c_str());
	if(!dir)
	{
		f.str("");
		f << "mkdir -p ./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps 
			<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "_stemCellRadius=" << stemCell_initialRadius << "/seed=" << _seed << "/animation";
		system(f.str().c_str());
	}
	...
	r_birth = turnover_rate * log(2.0);

	// Mutation (now decoupled from division) rate given as command line argument (as the fraction of r_birth i.e. argument of 2 results in r_mut = 2*r_birth)
	r_mut = mutation_ratio * r_birth;

	// Death rate 
	r_death = r_birth * bdratio;



	// Compute normalised reaction rates
	r_birth_normalised = r_birth/(r_birth + r_mut + r_death);
	r_mut_normalised = r_mut/(r_birth + r_mut + r_death);
	r_death_normalised = r_mut/(r_birth + r_mut + r_death);
...
/*
	...
	g.str("");
	g << "./DATA/maxSize=" << _maxsize << "_neighbourhood=" << NEIGHBOURHOOD << "_stemCell=" << stemCell << "_sweeps=" << sweeps << "_ut=" << _ut
			<< "_BDratio=" << bdratio << "_rBirth=" << r_birth << "/" << _seed << "/net_flux_chain_displacement_scalar_products_data_scatter.dat";
	netflux_file.open(g.str().c_str());
*/


	// Array to accumulate "flux" data online
	float * flux = new float[40];
	for (int i = 0; i < 40; ++i)
	{
		flux[i] = 0.0;
	}

	int * flux_count = new int[40];
	for (int i = 0; i < 40; ++i)
	{
		flux_count[i] = 0;
	}
	...
	//vector<int> new_mutation;
	//new_mutation.push_back(liver[radius][radius][radius].cellID);
	//mutations.push_back(new_mutation);
	...
	*/
	...
	do
	{
		...
		surface_division(liver , mutations , cell_x , cell_y , &Ntot , &x_b , &y_b , radius , &next_cellID );
	...
	*/
	...
	// Fill centre circle (up to specified radius) with stem cells
	for (int i = 0; i < (2*radius); i++)
	{
		for (int j = 0; j < (2*radius); j++)
		{
			if (liver[i][j].cellID != -1)
			{
				...
				if (dist <= stemCell_initialRadius*radius_double/100.0)
				{
					liver[i][j].setCellType(1);
...
/*
	if (stemCell == 1)
	{
		cout << "Stem cell option chosen" << endl;
		...
		for (int i = -1; i < 2; i++)
		{
			for (int j = -1; j < 2; j++)
			{
					...
					//cout << liver[radius+i][radius+j].cellID << endl;

					//cout << "Setting cell to stem cell" << endl;
					liver[radius+i][radius+j].setCellType(1);
					liver[radius+i][radius+j].setID(next_cellID);
...
*/
	...
	f.str("");
	f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps
			<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "_stemCellRadius=" << stemCell_initialRadius << "/seed=" << _seed << "/animation/000.csv";
	...
	for (int sweep = 0; sweep < sweeps; ++sweep)
	{
		...
		do
		{
			...
			while ((liver[cell_x][cell_y].cellID == -1) || (liver[cell_x][cell_y].stem_cell == 1));
		...
		do
		{
			...
			MUTATION = false;
			DEATH = false;
			if (drand48() < r_birth_normalised)
			{
				BIRTH = true;
			}
			else if (drand48() < (r_birth_normalised + r_mut_normalised))
			{
				MUTATION = true;
			}
			else
			...
			// Uncomment this to forbid stem cells from mutating
			//if ((MUTATION == true) && (liver[cell_x][cell_y].stem_cell == true))
			//{
			//	continue;
			//}
			...
			do
			{
				...
				// Check if cell at (x,y) = (cell_x , cell_y) is a stem cell, and if so check whether it is at the 'edge' of the stem cell pool
				if (liver[cell_x][cell_y].stem_cell == 1)
				{
					// Count neighbouring stem cells
					stem_cell_neighbours = 0;
					for (int i = -1; i < 2; i++)
					{
						for (int j = -1; j < 2; j++)
						{

							if (liver[cell_x + i][cell_y + j].stem_cell == 1) stem_cell_neighbours += 1;
				...
				//cout << x_b << " " << y_b << " " << z_b << " " << 2*radius << " " << cell_x << " " << cell_y << " " << cell_z << " " << liver[cell_x][cell_y].stem_cell <<  endl;

			}
			while ( (liver[cell_x][cell_y].cellID == -1) || ((stemCell == 1) && (cell_x == radius) && (cell_y == radius)) || ((liver[cell_x][cell_y].stem_cell == 1) && (stem_cell_neighbours == NEIGHBOURHOOD + 1)) );


			ran = drand48();
			if (((BIRTH == true) || (DEATH == true)) && (liver[cell_x][cell_y].stem_cell == 0) && (ran <= division_ratio)) continue;
			//if (((BIRTH == true) || (DEATH == true)) && (liver[cell_x][cell_y].stem_cell == 0)) continue;



			//cout << "Iter #" << iter << " ---> birth=" << BIRTH << ", mutation=" << MUTATION << ", death=" << DEATH << ", Ntot = " << Ntot << endl;


			//cout << cell_x << " " << cell_y << " " << cell_z << " " << liver[cell_x][cell_y][cell_z].stem_cell <<  endl;
			//cout << "| Bounds=(" << x_b << ", " << y_b << ", " << z_b << ") | Chosen cell=(" << cell_x << ", " << cell_y << ", " << cell_z << ") | N(cells)=" << Ntot << " | Radius=" << radius;
			...
			if (DEATH)
			{
				...
				if (liver[cell_x][cell_y][cell_z].stem_cell == 0)
				{

					if (liver[cell_x][cell_y][cell_z].stem_cell == 1) cout << "stem cell died" << endl;
					...
					for (int m = 0; m < mutations.size(); ++m)		// Loop over all mutations present in liver
					{
						for (int q = 0; q < mutations[m].size(); ++q)
						{
							if ( mutations[m][q] == liver[cell_x][cell_y][cell_z].cellID )
							{
								mutations[m].erase (mutations[m].begin()+q);
							}
						}
					...
					// Delete cell from lattice
					liver[cell_x][cell_y][cell_z].setID(-1);

					// Size of liver is reduced by 1
					Ntot -= 1;
					--counter;

					//cout << " --> cell died --> " << liver[cell_x][cell_y][cell_z].cellID << endl;

					// Progress time variable
					//t += -log(1 - drand48())/(r_surv * Ntot);
			...
			*/
			...
			if (BIRTH)
			{
				...
				//if (liver[cell_x][cell_y][cell_z].stem_cell == 1)
				//{
				//	cout << "Stem cell divided......." << endl;
				//}
				...
				do
				{
					volumetric_division(liver , mutations , cell_x , cell_y , &Ntot , &x_b , &y_b , radius , &next_cellID , flux , flux_count);
				}
			...
			// Progress time variable
			t += -log(1-drand48())/( (r_birth + r_mut + r_death)*Ntot );
			...
			if ((Ntot%1000 == 0) && (BIRTH))
			{
				cout << "\r\tIter=" << iter << ", N=" << Ntot << ", # mutations = " << mutations.size() << endl;
			}
			...
			if (iter%10000 == 0)
			{
				...
				if (!quiet)
				{
					...
					//cout << "\r\tIter=" << iter << ", N=" << Ntot << ", # mutations = " << mutations.size() << ": Bounds -> " << x_b << ", " << y_b << ", " << z_b << endl;
		...
		{
			f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps 
				<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "/seed=" << _seed << "/animation/00" << sweep << ".csv";
		}
		else if (sweep < 100)
		{
			f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps 
				<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "/seed=" << _seed << "/animation/0" << sweep << ".csv";
		}
		else
		{
			f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps 
				<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "/seed=" << _seed << "/animation/" << sweep << ".csv";
		}
...
/*
	...
	f.str("");
	f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps
			<< "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "/seed=" << _seed << "/liver.csv";
...
/*
	...
	for (int i = 0; i < 40; ++i)
	{
		netflux_file << ((float)(i+1)*init_rad/40.0) << " " << flux[i]/(float)flux_count[i] << endl;
	}
*/
	

	// ----> Choose to write full mutation data i.e. string of 1's and 0's for each mutation for each cell
...
*/
	...
	// Write final spatial data to file
	f.str("");
	f << "./2D_DATA/maxSize=" << _maxsize << "_stemCell=" << stemCell << "_sweeps=" << sweeps 
                                << "_BDratio=" << bdratio << "_mutationRatio=" << mutation_ratio << "_1minusA=" << division_ratio << "_stemCellRadius=" << stemCell_initialRadius << "/seed=" << _seed << "/animation/" << "final_spatialData.csv";
